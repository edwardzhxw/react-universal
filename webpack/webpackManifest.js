  ...
  compiler.plugin('emit', function(compilation, compileCallback) {
    ...
    _.merge(manifest, compilation.chunks.reduce(function(memo, chunk) {
      ...
      return chunk.files.reduce(function(memo, file) {
        ...
        if (chunkName && file.indexOf('chunk') === -1 && /(js|css)$/.test(this.getFileType(file))) {
          memo[this.getFileType(file)].push(file);
        } else {
          memo[file] = file;
        }
        return memo;
    ...
    if (this.opts.basePath) {
      manifest = _.reduce(manifest, function(memo, value, key) {
        if (key === 'js' || key === 'css') {
          memo[key] = value.map(v => this.opts.basePath + v);
        } else {
      ...
      manifest = _.reduce(manifest, function(memo, value, key) {
        if (key === 'js' || key === 'css') {
          memo[key] = value.map(v => this.opts.publicPath + v);
        } else {
